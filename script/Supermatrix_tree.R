...
system(paste0('raxmlHPC-SSE3 -f a -m GTRGAMMA -T 2 -# 100 -p ',
              sample(0:10000000, 1), ' -x ', sample(0:10000000, 1),
              ' -n gouqi -s ', inpt, ' -q ', prttnfl))
# consensus
system('raxmlHPC-SSE3 -m GTRCAT -J MR -z RAxML_bootstrap.gouqi -n gouqi_con')

# CLEAN-UP
file.rename('RAxML_bestTree.gouqi', file.path(wd, 'best_tree.tre'))
file.rename('RAxML_bootstrap.gouqi', file.path(wd, 'bootstraps.tre'))
file.rename('RAxML_MajorityRuleConsensusTree.gouqi_con',
            file.path(wd, 'consensus.tre'))
file.remove(list.files(pattern='RAxML.*\\.gouqi'))